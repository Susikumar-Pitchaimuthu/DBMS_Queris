...
/*
SQL is case-i sensitive.alter

But we usually use UPPERCASE for keywords

And Lower cases for values.
...
SELECT name,product_id,unit_price , unit_price*1.1 AS 'new price' FROM products;
...
FROM customers
WHERE points > 1000 OR birth_date > '1889-01-01' AND state = 'VA';
 ...
 */
 ...
 WHERE last_name REGEXP  "^Mac|field$";
  ...
  SELECT* 
 FROM customers
 WHERE last_name REGEXP  "ae|be|ce|de|ef|ge|he";
...
-- The orders of the matters. SELECT ->FROM -> optionally ORDER BY -> LIMIT 

-- Not following the order dives error. 
...
LIMIT 9 ,1;
 ...
 -- count all salaries for employess payable
 ...
 SELECT MAX(SALARY) FROM employees;
  ...
  SELECT MIN(SALARY) FROM employees;
   ...
   --  Average salaries and employees count
   
   SELECT AVG(SALARY) FROM employees; 
   SELECT COUNT(EMPLOYEE_ID) FROM employees;
 
 --  Count employees who all are working in the company
  SELECT COUNT(JOB_ID) FROM employees;
  
  
  --   get the number of distinct jobs available in the employees table
...
SELECT e.department_id, city,street_address
FROM departments AS e INNER JOIN locations AS d
...
SELECT e.job_id, first_name, last_name, hire_date
...
-- Write a query to get the department name and number of employees in the department. (Requires GROUP BY)SELECT 
...
FROM employees AS e INNER JOIN jobs AS d
...
INSERT INTO user (username, first_name, last_name,email, password,date_of_birth,phone,whatsapp_no)
VALUES ('Siva bharathi', 'Nagaraj','Punniyavan','punniyavan@gmail.com', '12345@smSM', '1990-02-02', 984329409, 9524329409);
...
CREATE TABLE IF NOT EXISTS CFU
...
SELECT  d.max_salary - min_salary, job_title, first_name, last_name
FROM employees AS e INNER JOIN jobs AS d
ON e.job_id = d.job_id;

-- Write a query to find the employee ID, job title, number of days between ending date and starting date for all jobs in department 90

SELECT employees.employee_id, jobs.job_title, job_history.end_date - job_history.start_date AS Days
FROM ((employees
...
SELECT  d.department_name, salary / employee_id AS Average
FROM employees AS e INNER JOIN departments AS d
ON e.department_id = d.department_id;
...
SELECT e.department_name, city,first_name
FROM departments AS e INNER JOIN locations AS d
ON e.location_id = d.location_id;


--  Write a query for count a employees from the all the department

SELECT department_id, COUNT(*) AS "count of employees"
FROM employees
